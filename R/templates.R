...
use_template_1 <- function(eligible = 0, randomized = 0, timepoints = 2, labels = c("Timepoint 1", "Timepoint 2"),
                      descriptions = c("Baseline Testing", "Post-intervention"), intervention = "Intervention", 
                      intervention_n = 0, control = "Control", control_n = 0, verbose = FALSE){
  
  if(timepoints == 0) stop("You must provide a positive integer for the timepoints argument.")
  
  timepoint_names <- paste0("t", 1:timepoints)
  if(length(labels) != timepoints) stop("The length of the labels must equal timepoints.")
  ...
  if(length(intervention_n) != timepoints & all(intervention_n != 0)) {
    stop(sprintf("Expecting %s N values for intervention_n but only %s found.", timepoints, length(intervention_n)))
  } else if(all(intervention_n == 0)){
    intervention_n <- rep(intervention_n, timepoints)
  }
  
  if(length(control_n) != timepoints & all(control_n != 0)) {
    stop(sprintf("Expecting %s N values for control_n but only %s found.", timepoints, length(control_n)))
  ...
  create_graph() |>
    set_node_parameters(
      ...
      width = 3.5, height = 0.5, fillcolor = "White") |>
    add_node(name = c("eligible", "randomized"),
             label = c(sprintf("Eligible (n = %s)", eligible),
                       sprintf("Randomized (n = %s)", randomized)),
             pos = c(4, 8, 4, 7)) |>
    add_node(name = c("i1", "i2"), pos = c(1.5, 7, 6.5, 7), width = 0, 
             height = 0, style = "invis", label = c("", ""), shape = "square") |>
    add_node(name = timepoint_names, label = labels,
             pos = timepoint_positions[1:(2*timepoints)], color = "White", width = 0) |>
    add_node(name = "intervention", label = sprintf("Allocated to %s", intervention), pos = c(1.5, 6.5),
             color = "White", height = 0.25) |>
    add_node(name = "control", label = sprintf("Allocated to %s", control), pos = c(6.5, 6.5),
             color = "White", height = 0.25) |>
    add_node(name = group_names[1:timepoints],
             label = paste0("Completed ", tolower(descriptions), " (n = ", intervention_n, ")"),
             pos = group_positions$intervention[1:(2*timepoints)]) |>
    add_node(name = group_names[(timepoints+1):length(group_names)],
             label = paste0("Completed ", tolower(descriptions), " (n = ", control_n, ")"),
             pos = group_positions$control[1:(2*timepoints)]) |>
    set_edge_parameters(color = "Black", arrowhead = "normal", arrowtail = "none",
                        style = "filled", tailport = "center") |>
    ...
    add_edge(from = group_names[1:timepoints][1:(length(group_names[1:timepoints])-1)],
             to = group_names[1:timepoints][2:length(group_names[1:timepoints])]) |>
    add_edge(from = group_names[(timepoints+1):length(group_names)][1:(length(group_names[(timepoints+1):length(group_names)])-1)],
             to = group_names[(timepoints+1):length(group_names)][2:length(group_names[(timepoints+1):length(group_names)])], 
             verbose = verbose)
}
